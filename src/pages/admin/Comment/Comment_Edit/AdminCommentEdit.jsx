...
const AdminCommentEdit = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(`${Constanst.DOMAIN_API}/api/admin/reviews/${reviewId}/status`, {
                method: 'PATCH',
                headers: {
            ...
            navigate('/admin/comment'); // Điều hướng về trang danh sách
        } catch (err) {
            ...
            <div className="card">
                <div className="card-body">
                     ...
                     {review.images && review.images.length > 0 && (
                        <div>
                            ...
                            {review.images.map(img => (
                                <img key={img.id} src={`${Constanst.DOMAIN_API}/uploads/${img.image_url}`} alt="review" width="100" className="img-thumbnail" />
                            ))}
                            </div>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="mb-3">
                            <label htmlFor="status" className="form-label"><strong>Cập nhật trạng thái:</strong></label>
                            <select 
                                id="status" 
                        ...
                        <button type="button" onClick={() => navigate('/admin/comment')} className="btn btn-secondary ms-2">
                            Hủy
                        </button>
                    </form>
                </div>
