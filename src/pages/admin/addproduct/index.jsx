...
const AddProduct = () => {
  ...
  const [categoryParents, setCategoryParents] = useState([]); // Danh mục cha (parent_id === null)
  const [categories, setCategories] = useState([]); // Danh mục con của cha được chọn
  ...
  // Khi chọn danh mục cha, fetch danh mục con
  useEffect(() => {
    setValue("categoryparent_id", selectedParentId); // Nếu BE còn trường này thì giữ, nếu không thì bỏ
                ...
                <div key={field.id} className="border rounded p-2 mb-2">
                  <div className="row">
                    ...
                    <div className="col-auto">
                      <button type="button" className="btn btn-danger btn-sm" onClick={() => removeVariation(idx)}>Xóa
                      </button>
                    </div>
                  </div>
                </div>
            ))}
            <button type="button" className="btn btn-primary btn-sm" onClick={() => appendVariation({})}>Thêm biến thể
            </button>
          </div>
