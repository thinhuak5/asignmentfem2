...
const calcTotals = (order) => {
    ...
    const totalForDisplay = backendTotal > 0 ? backendTotal : Math.max(0, subtotal - discount);
...
const OrderList = () => {
    ...
    const fetchOrders = async () => {
        ...
        setError(null);
        try {
            const res = await adminApi.get("/orders"); // GET /api/admin/orders
            ...
            setError(err?.response?.data?.message || err.message || "Lỗi khi lấy dữ liệu đơn hàng");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handlePaymentStatusChange = (id, value) =>
        setUpdatedPaymentStatus({...updatedPaymentStatus, [id]: parseInt(value, 10)});
    const handleOrderStatusChange = (id, value) =>
        setUpdatedOrderStatus({...updatedOrderStatus, [id]: parseInt(value, 10)});
                                    ...
                                    <td>
                                        {editingOrderId === order.id ? (
                                            order.payment_id === 2 ? (
                                                ...
                                                <Form.Select
                                                    value={updatedPaymentStatus[order.id] ?? order.payment_status}
                                                    onChange={(e) =>
                                                ...
                                                {orderStatusOptions
                                                    .filter(
                                                        (opt) =>
                                                            opt.value === "" ||
                                                            opt.value === "0" ||
                                                            parseInt(opt.value, 10) >= order.status
                                                    )
                                                    .map((option) => (
                            ...
                            {detailLoading ? (
                                ...
                                <>
                                    <div className="mb-2">
                                        ...
                                        {paymentBadge(detailOrder.payment_id, detailOrder.payment_status)} <br/>
                                                ...
                                                return (
                                                    <tr>
                                                        ...
                                                        <td className="fw-bold text-danger">
                                                            ...
                                                            {discount > 0 && (
                                                                <span
                                                                    className="text-muted"> (-{fmtMoney(discount)} đ)</span>
                                                            )}
                                                        </td>
                                                    </tr>
                                                );
